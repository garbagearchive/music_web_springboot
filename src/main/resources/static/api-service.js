...
class ApiService {
    ...
    async searchSongs(searchTerm = '', genreId = null) {
        let url = '/songs';
    ...
    transformSongData(backendSong) {
        return {
            ...
            artist: backendSong.artist?.name || 'Unknown Artist',
...
// Biến toàn cục cho bộ lọc tìm kiếm
let currentGenreFilter = null;
// let currentArtistFilter = null; // Removed
// Hàm fetch mới với tham số artist
async function fetchAndRenderSongsWithArtistParam(searchTerm = '', genreFilterValue = null, artistParam = null) {
    const searchResultsDiv = document.getElementById('searchResults');
    searchResultsDiv.innerHTML = '<div class="loading"></div>';
    ...
    try {
        ...
        if (genreFilterValue !== null) {
            params.append('genre', genreFilterValue);
        }
        if (artistParam) {
            params.append('artist', artistParam);
        }
        ...
        const songs = await apiService.makeRequest(url);
        const transformedResults = songs.map(song => apiService.transformSongData(song));
        displayTracks(transformedResults, 'searchResults');
    } catch (error) {
        console.error('Error fetching songs:', error);
        searchResultsDiv.innerHTML = '<p>Failed to load songs.</p>';
    }
}
...
async function initializeApp() {
    try {
        ...
        const noArtistOption = document.createElement('li');
        noArtistOption.textContent = 'Only songs without artist';
        noArtistOption.addEventListener('click', () => {
        currentGenreFilter = null; // Không lọc genre
        genreBtn.textContent = 'No Artist ▼';
    // Gửi kèm param artist=none
        fetchAndRenderSongsWithArtistParam(searchInput.value, null, 'none');
        dropdown.style.display = 'none';
    });
    dropdown.appendChild(noArtistOption);
    } catch (error) {
...
// Music player functions (giữ nguyên)
...
async function fetchAndRenderSongs(searchTerm = '', genreFilterValue = null) {
    const searchResultsDiv = document.getElementById('searchResults');
    searchResultsDiv.innerHTML = '<div class="loading"></div>';
    ...
    try {
        const songs = await apiService.searchSongs(searchTerm, genreFilterValue);
        const transformedResults = songs.map(song => apiService.transformSongData(song));
        displayTracks(transformedResults, 'searchResults');
    } catch (error) {
        console.error('Error fetching songs:', error);
...
function setupSearch() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        searchTimeout = setTimeout(() => {
            fetchAndRenderSongs(searchTerm, currentGenreFilter);
        }, 300);
    });
    ...
    clearSearchBtn.addEventListener('click', () => {
        ...
        fetchAndRenderSongs('', currentGenreFilter);
    });
    
    async function populateGenreDropdown() {
        const dropdown = document.getElementById('genreDropdown');
        const genreBtn = document.getElementById('genreFilter').querySelector('.filter-btn');
        dropdown.innerHTML = '';
        ...
        allOption.addEventListener('click', () => {
            ...
            fetchAndRenderSongs(searchInput.value, currentGenreFilter);
            dropdown.style.display = 'none';
        });
        ...
        try {
            ...
            genres.forEach(genre => {
                ...
                li.addEventListener('click', () => {
                    currentGenreFilter = genre.genreID;
                    genreBtn.textContent = genre.genreName + ' ▼';
                    fetchAndRenderSongs(searchInput.value, currentGenreFilter);
                    dropdown.style.display = 'none';
                });
            ...
            unknownOption.addEventListener('click', () => {
                currentGenreFilter = 'unknown';
                genreBtn.textContent = 'Unknown ▼';
                fetchAndRenderSongs(searchInput.value, currentGenreFilter);
                dropdown.style.display = 'none';
            });
            ...
            errorLi.textContent = 'Failed to load genres.';
            dropdown.appendChild(errorLi);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('genreFilter').querySelector('.filter-btn').addEventListener('click', (e) => {
        e.stopPropagation();
        const dropdown = document.getElementById('genreDropdown');
